...
class Chaingun implements Gun {
    ...
    shoot() {
        ...
        this._ui.guns[this._ui.currentGun].image.sourceLeft = arr[this._ui.index]
        this._ui.guns[this._ui.currentGun].image.top = (0.05) * Math.sin(2 * (0.05 * this.moveTick)) + 0.2;
    }
...
class Shotgun implements Gun {
    ...
    stopMovingGun() {
        ...
        var imageLeftInt = parseInt(left.match(/(.+)px/)![1])
        if (imageLeftInt > 0) {
            imageLeftInt -= 1;
            this.image.left = imageLeftInt;
        }
        ...
        var imageTopInt = parseInt(top.match(/(.+)%/)![1])
...
export class UIManager {
    ...
    display(type: DisplayType) {
        ...
        const array = this[type].toString().split('').map((e) => {
            return parseInt(e);
        });
